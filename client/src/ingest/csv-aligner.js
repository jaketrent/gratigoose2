...
import css from './csv-aligner.module.css'
...
class CsvAligner extends React.Component {
  ...
  handleColumnDrop(index, name) {
    ...
    if (indexInColumns > -1) columns.splice(indexInColumns, 1)

    const selectedColumns = [...this.state.selectedColumns]
    const indexInSelected = selectedColumns.indexOf(name)
    if (indexInSelected > -1) selectedColumns[indexInSelected] = undefined
      ...
      <div className={this.props.css.root}>
        <div className={this.props.css.controls}>
          <div className={this.props.css.options}>
            ...
            <CsvHeaderToggle
              checked={this.state.includesHeader}
              onChange={this.handleToggleIncludesHeader}
            />
          </div>
          <div className={this.props.css.submit}>
            <button className={this.props.css.btn} onClick={this.handleSubmit}>
              Submit
            </button>
          </div>
        </div>
