...
import css from './csv-table.module.css'
...
function renderRow(props, row, i) {
  return <tr key={i}>{renderCells(props, row)}</tr>
}
...
function Column(props) {
  const className = props.isOver ? props.css.colHover : props.css.col
  return props.connectDropTarget(
    <td className={className}>
      {props.name ? (
        <CsvColumnOption
          css={{ root: props.css.optionRootDropped }}
          name={props.name}
        />
      ) : null}
    </td>
