...
class PlanVsActivityVizD3 {
  ...
  initScales() {
    ...
    const y = d3.scaleLinear().range([this.getChartHeight(), 0])
  ...
  setDomain() {
    this.config.x0.domain(this.getData().map(d => d.x))

    this.config.x1
      .range([0, this.config.x0.bandwidth()])
      .domain(this.getData()[0].ys.map((_, i) => i))

    this.config.y.domain([0, d3.max(this.getData(), d => d3.max(d.ys, y => y))])
  }
  clear() {
  ...
  drawBars() {
    ...
    const areas = svg.selectAll('.' + this.config.css.area).data(this.getData())
  ...
  drawBarLabels() {
    ...
    const labels = svg.selectAll('.' + css.barLabel).data(bars.data())
    ...
    labels.transition().call(position)
  }
  draw(data, config) {
    this.clear()
    this.drawSvg()
